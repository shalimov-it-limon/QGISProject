...
class LasTableModel(QAbstractTableModel):
    ...
    def __init__(self, path):
        super().__init__()
        self.lasio_file = lasio.read(path)
        ...
        self.filename = "E:\\Work\\QGISProject\\Homework4\example.las"
    ...
    def insertRows(self, row: int, count: int, parent: QModelIndex = ...) -> bool:
        ...
        rows = pandas.DataFrame(
            np.zeros((count, self.columnCount())),
    ...
    def insertColumns(self, column: int, count: int, parent: QModelIndex = ...) -> bool:
        try:
            for i in range(count):
                self.las.insert(column, "NEW_" + str(i), np.zeros((255)))
        except IndexError:
            self.las["NEW"] = np.zeros((255))
        finally:
            self.saveData()

    def saveData(self):
        df = self.las.set_index('DT', drop=True)
        # self.lasio_file.set_data_from_df(df)
...
table = LasTableModel("E:\\Work\\QGISProject\\Homework4\example.las")
table.insertRows(233, 10)
table.insertRows(234, 10)
table.insertColumns(1, 10)
